...
# Handle enroll
$out_dir_enroll = "$out_dir/yiwei_enroll";
...
open(META, "<$db_base/docs/train_ndx") or die "cannot open wav list";
%utt2fixedutt = ();
while (<META>) {
  $line = $_;
  @toks = split(" ", $line);
...
if (system("find $db_base/data/train/ -name '*.wav' > $tmp_dir_enroll/sph.list") != 0) {
...
# Handle test
$out_dir_test= "$out_dir/yiwei_test";
...
if (system("find $db_base/data/test/ -name '*.wav' > $tmp_dir_test/sph.list") != 0) {
  die "Error getting list of sph files";
}

open(KEY, "<$db_base/docs/trial_ndx") || die "Could not open trials file $db_base/docs/sre16_eval_trial_key.tsv.  It might be located somewhere else in your distribution.";
